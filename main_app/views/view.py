...
def indexPanelView(request):
    ...
    related_active_order_id_to_current_user = order_process_action.objects.filter(process_action__action__in=action_perms).filter(status=under_check).values_list('order_id').all()
    active_order_duties = order.objects.filter(pk__in=related_active_order_id_to_current_user).order_by('-priority').all()
